...
ZGI product architecture · four-layer design

Multi-level permissions and auditing

Refined department structure management supports independent budget accounting by department to avoid Token abuse

Database Enhanced Knowledge Base RAG

Introducing a database to optimize the knowledge base and achieve a recall accuracy rate of more than 99%

One-click access to global models

Pre-register the mainstream large models in the market (dialogue, rearrangement, vector, image, etc.), users do not need to configure cumbersome API Keys one by one

Own intellectual property rights

The ZGI core code has completely independent copyright and is not re-encapsulated, avoiding the harsh restrictions of open source protocols and the risk of underlying dependencies.
...
Preventing accidental deletion and inheritance of assets

There is a check mechanism to prevent accidental deletion of running processes or intelligent assets; when employees leave, AI assets can be transferred with one click to ensure the continuity of corporate digital assets.

Knowledge graph enhanced RAG

Organize fragmented knowledge into a graph structure so that AI can understand the correlation between knowledge. During retrieval, nearby nodes can be recalled in association to solve the information fragmentation caused by traditional block retrieval.

Local/self-built model support

Supports enterprises to configure their own API Key for locally deployed large models, is compatible with existing IT architecture, and does not lock in model suppliers.

Golang high performance backend

In view of Python's natural shortcomings in multi-concurrency support, ZGI uses Golang to write the underlying code, which can stably support the real business needs of multiple customers and large-scale users online at the same time.
